...
export default function Home() {
  ...
  const refreshProgress=async()=>{const response=await fetch("/api/progress",{cache:"no-store"});if(!response.ok)throw new Error("Không thể tải tiến độ.");const data=await response.json();setAccount(data.user);setRatings(data.progress.ratings||{});setHistory(data.progress.history||[]);setStartDate(data.progress.startDate||dateKey(new Date()))};
  const submitAuth=async(e:React.FormEvent)=>{e.preventDefault();setAuthBusy(true);setAuthError("");try{const response=await fetch(`/api/auth/${authMode}`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({username,password,confirmPassword})});const data=await response.json();if(!response.ok)throw new Error(data.error||"Không thể tiếp tục.");await refreshProgress();setAuthOpen(false);setPassword("");setConfirmPassword("");setAuthBusy(false)}catch(error){setAuthError(error instanceof Error?error.message:"Không thể tiếp tục.");setAuthBusy(false)}};
  const logout=async()=>{await fetch("/api/auth/logout",{method:"POST"});setAccount(null);setRatings({});setHistory([]);setStartDate(dateKey(new Date()));setSaveState("idle")};
